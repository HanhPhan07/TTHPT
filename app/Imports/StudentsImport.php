    ...
    */
    ...
    public function collection(Collection $rows)
    {
        foreach ($rows as $row) {
            ...
            $subject=explode( ', ', $row['mon_thi']);
            $subject_array=Subject::WhereIn('name',$subject)->get()->pluck('id')->toArray();
            if($subject_array!=null){
                for($i=0;$i<count($subject_array);$i++){
                    $student->examSubject()->attach($subject_array[$i]);
                }
            }
            $school=School::whereName($row['truong'])->get()->pluck('id')->toArray();
            $student->schoolList()->create([
                'id_school'=>$school[0],
            ]);
        }
